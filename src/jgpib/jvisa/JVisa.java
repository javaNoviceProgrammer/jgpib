...
//  public static final String JNA_LIBRARY_NAME = "visa";
  /** instance of loaded VISA library
    A user of this library should check this variable not being null after instantiation */
  public static JVisaInterface visaLib;
  /** libreVisa has not implemented the entire API. For example, only one
   * attribute is implemented. This flag allows an application to distinguish.
   */
...
//        visaLibName = "/home/gfuchs/Projects/LibreVISA/librevisa/src/.libs/libvisa.so";
...
//      LOGGER.info(JVISA_VERSION);
   ...
   *                   for example TCPIP::192.168.1.106::INSTR
   * @return status of the operation
   */
   ...
   *                      This parameter is only used under Linux / libreVisa.
   * @return status of the operation
   * @throws jvisa.JVisaException if viRead does not succeed
   */
...
//      response.returnBytes = baos.toByteArray();      
  ...
  /**
   * This method reads a byte array from the instrument, usually a command response.
   * It is used when the expected number of bytes in the response is unknown or
   * or a Windows VISA library is used.
   * @param response response byte array
   * @param bufferSize size of response buffer in bytes
   * @return status of the operation
   * @throws jvisa.JVisaException if viRead does not succeed
   ...
   * The corresponding VISA function is not implemented in the libreVisa library.
   * @return status of the operation
   */
  public long clear() {
    if (isLibreVisa)
